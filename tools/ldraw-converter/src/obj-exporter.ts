...
export class ObjExporter {
    ...
    private async processModelData(
        ...
        for (const step of data.steps) {
            for (const subPart of step.parts) {
                ...
                console.log(`[ObjExporter Debug] Part: ${subPart.partId} | ID: ${subTargetID} | Pos: ${subPart.position.x},${subPart.position.y},${subPart.position.z}`);
    ...
    public async exportModel(modelData: RuntimeModelData, outputPath: string, offset: { x: number, y: number, z: number } = { x: 0, y: 0, z: 0 }) {
        this.objectGroups.clear();
        const identity: Matrix4x4 = {
            c0: { x: 1, y: 0, z: 0, w: 0 },
            c1: { x: 0, y: 1, z: 0, w: 0 },
        ...
        await this.writeObjResponse(outputPath, offset);
    }
    ...
    private async writeObjResponse(outputPath: string, offset: { x: number, y: number, z: number } = { x: 0, y: 0, z: 0 }) {
        const lines: string[] = [];
        lines.push("# Generated by LDraw-Converter Custom Exporter (Split Objects)");
        lines.push("mtllib colors.mtl");
        ...
        for (const objectID of sortedIDs) {
            ...
            for (const face of group.faces) {
                ...
                // Indices are already 1-based RELATIVE to this object's vertex list start
                // We need to add globalVertexOffset
            ...
            for (const color of sortedColors) {
                lines.push(`usemtl code_${color}`);
                lines.push(...facesByColor.get(color)!);
            }
