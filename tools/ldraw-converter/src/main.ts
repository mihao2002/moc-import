...
async function main() {
                ...
                if (!models.has(part.partId)) {
                    ...
                    for (const resolvedPath of possiblePaths) {
                        if (!processedFiles.has(resolvedPath.toLowerCase()) && fs.existsSync(resolvedPath)) {
                            ...
                            console.log(`Found external submodel: ${path.basename(resolvedPath)}`);
    ...
    if (fs.existsSync(colorPath)) {
        console.log(`Parsing colors from ${colorPath}...`);
        ...
        // Generate colors.mtl
        console.log("Generating colors.mtl...");
        ...
        console.warn("LDConfig.ldr not found!");
    }
    ...
    if (argv.blender) {
        ...
        if (!fs.existsSync(argv.blender)) {
            console.error(`Blender executable not found at: ${argv.blender}`);
            process.exit(1);
        }
        renderer = new BlenderService(argv.blender);
        console.log("Using Blender Renderer");
    } else {
        const ldview = new LDViewService(ldviewPath, compiledLibraryPath);
        renderer = ldview;
        console.log("Using LDView Renderer");
    }
    ...
    const calculateBlenderCamera = (rotX: number, rotY: number, rotZ: number = 0, dist: number = 550, center: { x: number, y: number, z: number } = { x: 0, y: 0, z: 0 }): CameraTransform => {
        ...
        console.log(`[Debug] Inputs: rotX=${rotX} rotY=${rotY} rotZ=${rotZ}`);
        ...
        console.log(`[Debug] StartLoc: ${JSON.stringify(startLoc)}`);
        console.log(`[Debug] StartUp: ${JSON.stringify(startUp)}`);
        console.log(`[Debug] CamUpOrbit: ${JSON.stringify(camUpOrbit)}`);
        console.log(`[Debug] CamLocOrbit: ${JSON.stringify(camLocOrbit)}`);
    ...
    console.log("--- Phase 1: Baking Submodels to Temp Files ---");
    ...
    console.log("--- Phase 2: Exporting Assets ---");
                ...
                if (isSubmodel && submodelData) {
                    ...
                    if (!processedSubmodels.has(resolvedKey)) {
                        ...
                        try {
                            ...
                            console.error(`Failed to render submodel ${safeName}:`, e);
                        }
                    } else {
                        const outImgName = `${safeName}.png`;
                        (part as any).imageName = `submodels/${outImgName}`;
                    ...
                    if (partFile) {
                        ...
                        if (!isPrimitive) {
                            ...
                            if (!fs.existsSync(outImgPath)) {
                                try {
                                    ...
                                    console.error(`Failed to render part ${safeId}:`, e);
                                }
                            }
                        }
                    } else {
                        if (!processedParts.has(partKey)) {
                            processedParts.add(partKey);
                            console.warn(`Part file not found: ${part.partId}`);
                        }
                    }
                }
            }
    ...
    console.log("--- Phase 3: Generating Step Images ---");
    ...
    for (const stepModel of stepsDataList) {
        ...
        console.log(`Generating step images for Model: ${modelName}`);
        console.log(`Model ${modelName} has ${modelData.steps.length} steps.`);
        modelData.steps.forEach((s: any, idx: number) => {
            console.log(`  Step ${idx + 1}: ${s.parts.length} parts.`);
        });
        ...
        for (const step of modelData.steps) {
            ...
            if (renderer instanceof BlenderService) {
                ...
                // Logic to reference submodels correctly in the temp LDR
                const tempModels = await parser.parseModels(stepLdrPath);
                const tempModelData = tempModels.values().next().value;
                if (tempModelData) {
                    // Calculate Center (LDraw Coords)
                    centerX = (minX === Infinity) ? 0 : (minX + maxX) / 2;
                    centerY = (minY === Infinity) ? 0 : (minY + maxY) / 2;
                    centerZ = (minZ === Infinity) ? 0 : (minZ + maxZ) / 2;

                    // Centering Geometry: Pass center as offset to exportModel
                    await objExporter.exportModel(tempModelData, stepObjPath, { x: centerX, y: centerY, z: centerZ });
                    ...
                    console.log(`[Debug] Dimensions (LDraw): W(X)=${sizeX.toFixed(2)}, H(Y)=${sizeY.toFixed(2)}, D(Z)=${sizeZ.toFixed(2)}`);
                    console.log(`[Debug] Dimensions (Blender): W(X)=${blenderSizeX.toFixed(2)}, D(Y)=${blenderSizeY.toFixed(2)}, H(Z)=${blenderSizeZ.toFixed(2)}`);


                    try {
                        // Scan OBJ for true bounding box
                        const objContent = await fs.readFile(stepObjPath, 'utf-8');
                        ...
                        console.log(`[Debug] OBJ Geometry Bounds (Blender Space):`);
                        console.log(`[Debug] X: [${bMinX.toFixed(2)}, ${bMaxX.toFixed(2)}] Size: ${bSizeX.toFixed(2)}`);
                        console.log(`[Debug] Y: [${bMinY.toFixed(2)}, ${bMaxY.toFixed(2)}] Size: ${bSizeY.toFixed(2)}`);
                        console.log(`[Debug] Z: [${bMinZ.toFixed(2)}, ${bMaxZ.toFixed(2)}] Size: ${bSizeZ.toFixed(2)}`);
                        ...
                        (step as any).center = { x: centerX, y: centerY, z: centerZ };
                        (step as any).bounds = {
                            min: { x: bMinX + centerX, y: bMinY + centerY, z: bMinZ + centerZ },
                            max: { x: bMaxX + centerX, y: bMaxY + centerY, z: bMaxZ + centerZ }
                        };
